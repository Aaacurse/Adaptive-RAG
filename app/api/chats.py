...
async def create_chat(db:AsyncSession=Depends(get_db)):
    chat=await crud.create_chat(db)
    return chat

@router.get('/chats')
async def list_all_chats(db:AsyncSession=Depends(get_db)):
    chats=await crud.get_chats(db)
    return chats

@router.get('/chats/{chat_id}')
async def get_chat_by_id(chat_id:uuid.UUID,db:AsyncSession=Depends(get_db)):
...
async def update_chat_title(chat_id:uuid.UUID,body:UpdateTitleRequest,db:AsyncSession=Depends(get_db)):
    chat=await crud.get_chat(db,chat_id)
    if not chat:
        raise HTTPException(status_code=404,detail="Chat not found")
